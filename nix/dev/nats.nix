{lib, ...}: {
  perSystem = {pkgs, ...}: let
    ...
    # we need to wrap nsc and nats to ensure they pick up key related state from the data directory
    ...
    config.devshells.default = {
      ...
      devshell.startup = {
        setup-nats.text = ''
          ...
          nsc add account -n example
          nsc edit account -n example \
              --js-mem-storage -1 \
              --js-disk-storage -1 \
              --js-streams -1 \
              --js-consumer -1

          nsc add user -a example -n admin
          nsc add user -a example -n service

          nsc generate context -a example -u admin --context example-admin
          nsc generate context -a example -u service --context example-service
        '';
      };
